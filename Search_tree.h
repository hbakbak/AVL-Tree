	...
	private:
		class Node {
			public:
				...
				void updateFront(Type const &obj);
				void balanceNode(Node *&current_node);
				void rightRight(Node*& current_node);
				void leftLeft(Node*& current_node);
...
template <typename Type>
int Search_tree<Type>::Node::balancing_factor() {
	return (((right_tree)?right_tree->tree_height+1:0)
	-((left_tree) ? left_tree->tree_height+1:0));
}
...
bool Search_tree<Type>::insert( Type const &obj ) {
	if ( empty() ) {
		...
		tree_size = 1;
		//update head and tail pointers (back and front sentinel)
...
template <typename Type>
void Search_tree<Type>::Node::balanceNode(Node *&current_node) {
	current_node -> update_height(); 
	//check if balancing factor is less than -1 do a right rotation, otherwise do a left rotation
	if (current_node -> balancing_factor() < -1) {
		//if the current node is right heavy do a left rotation 
		if (current_node->left_tree->balancing_factor() > 0){
			leftLeft(current_node -> left_tree);
		}
		rightRight(current_node);
	}
	else if (current_node -> balancing_factor() > 1) {
		//if the current node is left heavy do a right rotation 
		if (current_node->right_tree->balancing_factor() < 0){
			rightRight(current_node -> left_tree);
		}
		leftLeft(current_node); 
	}
	//update tree height
...
template <typename Type>
void Search_tree<Type>::Node::rightRight(Node *&current_node) {
	Node *temp = new Search_tree<Type>::Node();
	temp = current_node -> left_tree; 
...
template <typename Type>
void Search_tree<Type>::Node::leftLeft(Node *&current_node) {
	Node *temp = new Search_tree<Type>::Node();
	temp = current_node -> right_tree; 
...
template <typename Type>
typename Search_tree<Type>::Node *Search_tree<Type>::Node::min(Search_tree<Type>::Node *root_node) {
	if(root_node==nullptr){
		return nullptr; 
	}
	while(root_node->left_tree){
		root_node = root_node -> left_tree; 
	}
	return root_node;
}

//insert the object into node 
template <typename Type>
bool Search_tree<Type>::Node::insert( Type const &obj, Search_tree<Type>::Node *&to_this ) {
	...
	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
			...
			update_height();
			//update next and previous pointers for iterator
